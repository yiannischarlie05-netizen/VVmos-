...
_ROOT = Path(__file__).resolve().parent.parent  # Go up to workspace root
...
CARD_HOLDER = "Isuru"
AGE_DAYS = 180
CMD_DELAY = 10  # seconds between commands — increased to avoid rate limiting

# VMOS Cloud Credentials
VMOS_AK = "YOUR_VMOS_AK_HERE"
VMOS_SK = "YOUR_VMOS_SK_HERE"
...
async def sh(cmd, label="", timeout=30, max_retries=3):
    """Execute shell command on VMOS device with proper polling and retry logic."""
...
# ═══ PHASE 0: APP INSTALLATION ══════════════════════════════════════
async def phase0_app_install():
    print("\n" + "=" * 70)
    print("PHASE 0: APP INSTALLATION (Gmail + Google Wallet)")
    print("=" * 70)
    
    # Check current state
    print("  Checking current app state...")
    gmail_check = await sh("pm path com.google.android.gm 2>/dev/null", "check_gmail")
    wallet_check = await sh("pm path com.google.android.apps.walletnfcrel 2>/dev/null", "check_wallet")
    
    gmail_installed = "package:" in (gmail_check or "")
    wallet_installed = "package:" in (wallet_check or "")
    
    print(f"  Gmail: {'INSTALLED ✓' if gmail_installed else 'NOT INSTALLED'}")
    print(f"  Google Wallet: {'INSTALLED ✓' if wallet_installed else 'NOT INSTALLED'}")
    
    # Install Gmail via Play Store if not installed
    if not gmail_installed:
        print("  Installing Gmail via Play Store...")
        # Try using Play Store to install
        install_cmd = "pm install-existing com.google.android.gm 2>/dev/null || echo NEED_INSTALL"
        out = await sh(install_cmd, "install_gmail")
        if "NEED_INSTALL" in out:
            print("  ⚠ Gmail not available as pre-installed, attempting market install...")
            # Alternative: use VMOS Cloud API to install app
            # For now, skip and continue - Gmail will be injected via account
            print("  ⚠ Skipping Gmail install - will inject via Google Account phase")
        else:
            print("  ✓ Gmail installed")
        await asyncio.sleep(CMD_DELAY)
    else:
        print("  ✓ Gmail already installed")
    
    # Install Google Wallet via Play Store if not installed
    if not wallet_installed:
        print("  Installing Google Wallet via Play Store...")
        install_cmd = "pm install-existing com.google.android.apps.walletnfcrel 2>/dev/null || echo NEED_INSTALL"
        out = await sh(install_cmd, "install_wallet")
        if "NEED_INSTALL" in out:
            print("  ⚠ Google Wallet not available as pre-installed, attempting market install...")
            # Alternative: use VMOS Cloud API to install app
            # For now, skip and continue - wallet injection will work without app
            print("  ⚠ Skipping Google Wallet install - wallet DB will be injected directly")
        else:
            print("  ✓ Google Wallet installed")
        await asyncio.sleep(CMD_DELAY)
    else:
        print("  ✓ Google Wallet already installed")
    
    # Final verification
    print("  Final verification...")
    gmail_final = await sh("pm path com.google.android.gm 2>/dev/null", "final_gmail")
    wallet_final = await sh("pm path com.google.android.apps.walletnfcrel 2>/dev/null", "final_wallet")
    
    gmail_ok = "package:" in (gmail_final or "")
    wallet_ok = "package:" in (wallet_final or "")
    
    print(f"  Gmail: {'✓' if gmail_ok else '✗'}")
    print(f"  Google Wallet: {'✓' if wallet_ok else '✗'}")
    
    print("  Phase 0 complete - proceeding with pipeline")
    return True
...
async def phase3_anomaly_patch():
    ...
    for cmd, label in patches:
        ...
        ok = out and "_OK" in out
        status = "✓" if ok else "✗"
        print(f"  {status} {label}: {out.strip()[:60] if out else 'NO OUTPUT'}")
        if ok: passed += 1
        await asyncio.sleep(CMD_DELAY)
...
# ═══ PHASE 5.5: DOWNLOAD APPS VIA PLAY STORE ════════════════════════
async def phase55_download_apps():
    print("\n" + "=" * 70)
    print("PHASE 5.5: DOWNLOAD APPS VIA PLAY STORE")
    print("=" * 70)
    
    # Check current state after account injection
    print("  Checking app state after account injection...")
    gmail_check = await sh("pm path com.google.android.gm 2>/dev/null", "check_gmail")
    wallet_check = await sh("pm path com.google.android.apps.walletnfcrel 2>/dev/null", "check_wallet")
    ...
    # Gmail should be auto-downloaded after account injection
    if not gmail_installed:
        print("  Gmail not auto-downloaded, attempting Play Store download...")
        # Open Play Store and search for Gmail
        await sh("am start -n com.android.vending/.AssetBrowserActivity -a android.intent.action.VIEW -d 'https://play.google.com/store/apps/details?id=com.google.android.gm'", "open_playstore_gmail")
        await asyncio.sleep(CMD_DELAY * 2)
        print("  ⚠ Manual Play Store download required - please install Gmail")
    else:
        print("  ✓ Gmail auto-downloaded after account injection")
    
    # Download Google Wallet via Play Store
    if not wallet_installed:
        print("  Downloading Google Wallet via Play Store...")
        await sh("am start -n com.android.vending/.AssetBrowserActivity -a android.intent.action.VIEW -d 'https://play.google.com/store/apps/details?id=com.google.android.apps.walletnfcrel'", "open_playstore_wallet")
        await asyncio.sleep(CMD_DELAY * 2)
        print("  ⚠ Manual Play Store download required - please install Google Wallet")
    else:
        print("  ✓ Google Wallet already installed")
    
    print("  Phase 5.5 complete - proceeding with pipeline")
    return True
...
async def main():
    global client
    client = VMOSCloudClient(ak=VMOS_AK, sk=VMOS_SK, base_url=VMOS_BASE_URL)
    ...
    # Phase 5.5: Download apps via Play Store (after account injection)
    await phase55_download_apps()
