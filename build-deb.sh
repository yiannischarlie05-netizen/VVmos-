...
#
# Updated: 2026-04-03 - Genesis integration fixes
#
# Output: dist/titan-vmos_13.0.1_amd64.deb
# ═══════════════════════════════════════════════════════════════════════
set -euo pipefail
...
rsync -a --exclude='__pycache__' --exclude='*.pyc' --exclude='_deprecated' \
    "${SRC_DIR}/vmos_titan/" "${STAGING}${INSTALL_PREFIX}/vmos_titan/"

# Wallet module
if [[ -d "${SRC_DIR}/wallet" ]]; then
    mkdir -p "${STAGING}${INSTALL_PREFIX}/wallet"
...
rsync -a --exclude='__pycache__' --exclude='*.pyc' --exclude='_deprecated' \
    "${SRC_DIR}/vmos_titan/api/" "${STAGING}${INSTALL_PREFIX}/server/"
